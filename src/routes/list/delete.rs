...
#[delete("/<id_list>")]
pub fn route(id_list :i64,token: BearerToken,db_pool: &State<Pool<SqliteConnectionManager>>) -> Result<(), Status> {
    ...
    if right_from_list(&conn, &caller_email, &id_list).map_err(|err| send_error(500,err.to_string()))? != 1{
        return Err(send_error(403, format!("can't delete list {} because you have not owner right",id_list) ));
    }


    conn.execute("DELETE FROM list WHERE id = ?", &[&id_list])

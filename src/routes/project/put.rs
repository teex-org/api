...
pub fn route(id: i64, body: Json<Body>, token: BearerToken, db_pool: &State<Pool<SqliteConnectionManager>>) -> Result<(), Status> {
    ...
    if right_from_project(&conn, &caller_email, &id).map_err(|err| send_error(500,err.to_string()))? != 1{
        return Err(send_error(403, format!("can't edit project {} because you have not owner right",id) ));
    }

    conn.execute("UPDATE project SET name = ? WHERE id = ?", &[&body.name,&id.to_string()]).map_err(|err| send_error(500, err.to_string()))?;
